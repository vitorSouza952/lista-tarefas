...
// Função que exibe tarefas ocultas

const exibirTarefas = () => {
    const tarefasAdicionadas = document.querySelectorAll('li');

    tarefasAdicionadas.forEach(tarefaAdicionada => tarefaAdicionada.style.display = 'flex');
...
// Evento de submissão do formulário de filtragem de tarefas
formularioFiltragem.addEventListener('submit', event => {
    ...
    if (tarefasAdicionadas.length > 0) {
        ...
        tarefasAdicionadas.forEach(tarefaAdicionada => {
            switch (valorCategoria) {
                case '1':
                    tarefaAdicionada.style.display = 'flex';

                    existemTarefas = true;

                    break;
                case '2':
                    if (tarefaAdicionada.classList.contains('concluida')) {
                        tarefaAdicionada.style.display = 'none';
                    } else {
                        tarefaAdicionada.style.display = 'flex';
        ...
        alternarModal('Erro: Não existem tarefas adicionadas.');
    }
});
