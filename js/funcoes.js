...
const alternarModal = (msg) => {
  [fundoModal, modal].forEach((elModal) =>
    elModal.classList.toggle("esconder")
  );

  msg !== ""
    ? (txtModal.textContent = msg)
    : setTimeout(() => (txtModal.textContent = msg), 300);
};

const limparCampoTarefa = () => {
  campoTarefa.value = "";
  window.innerWidth >= 992 ? campoTarefa.focus() : campoTarefa.blur();
};
...
const alternarEstadoTarefa = (botaoAlternarEstado) => {
  const itemListaTarefas = botaoAlternarEstado.closest(".item-lista-tarefas");

  itemListaTarefas.classList.toggle("concluida");

  itemListaTarefas.classList.contains("concluida")
    ? (botaoAlternarEstado.textContent = "Tornar Pendente")
    : (botaoAlternarEstado.textContent = "Concluir");
};

const deletarTarefa = (botaoDeletar) => {
  botaoDeletar.closest(".item-lista-tarefas").remove();
...
listaTarefas.addEventListener("click", (e) => {
  ...
  if (elAlvo.classList.contains("botao-alternar-estado"))
    alternarEstadoTarefa(elAlvo);

  if (elAlvo.classList.contains("botao-deletar")) deletarTarefa(elAlvo);
});

[fundoModal, botaoFecharModal].forEach((elModal) =>
  elModal.addEventListener("click", () => alternarModal(""))
);
